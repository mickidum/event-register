...
if ($safe_post['event_date']) {
  ...
  fwrite($temp_file_name, 'aitidim_event_reg_'.$event_date_file_name);
}

$table_content = fopen('table_content.txt', 'w'); 
$file = fopen('html/aitidim_event_reg_'.$event_date_file_name.'.html', 'a+'); 
$csv = fopen('csv/aitidim_event_reg_'.$event_date_file_name.'.csv', 'a+');
...
$html_content = "<tr><td>".(count(file('html/aitidim_event_reg_'.$event_date_file_name.'.html')) + 1)."</td>".$text_content."<td>".date("d-m-Y H:i:s")."</td></tr>\n";
